// import React, { useEffect, useCallback } from 'react';
// import { Button } from '../../button';
// import { ChevronLeft, ChevronRight, Maximize, Minimize } from 'lucide-react';
// import keyboardjs from 'keyboardjs';
// import Zoom from './zoom';
// import SliderNav from './slider-nav/slider-nav';
// import useScreenSize from '@/app-pdf/_hooks/use-screensize';
// import Share from '../../share';

// const Toolbar = ({ flipbookRef, containerRef, screenfull, pdfDetails, viewerStates, shareUrl, disableShare }) => {
//     const { width: screenWidth } = useScreenSize();
//     const pagesInFlipView = ((viewerStates.currentPageIndex + 1) % 2 === 0 && (viewerStates.currentPageIndex + 1) !== pdfDetails.totalPages)
//         ? `${(viewerStates.currentPageIndex + 1)} - ${viewerStates.currentPageIndex + 2}`
//         : (viewerStates.currentPageIndex + 1)

//     // Full screen >>>>>>>>>
//     const fullScreen = useCallback(() => {
//         if (screenfull.isEnabled) {
//             screenfull.toggle(containerRef.current, { navigationUI: "hide" });
//         }
//         screenfull.on('error', (event) => {
//             alert('Failed to enable fullscreen', event);
//         });
//     }, [screenfull, containerRef]);

//     // Keyboard shortcuts >>>>>>>>>
//     useEffect(() => {
//         const handleRight = () => flipbookRef.current.pageFlip().flipNext();
//         const handleLeft = () => flipbookRef.current.pageFlip().flipPrev();

//         keyboardjs.bind('right', null, handleRight);
//         keyboardjs.bind('left', null, handleLeft);
//         // keyboardjs.bind('f', null, fullScreen);

//         return () => {
//             keyboardjs.unbind('right', null, handleRight);
//             keyboardjs.unbind('left', null, handleLeft);
//             // keyboardjs.unbind('f', null, fullScreen);
//         };
//     }, [flipbookRef, fullScreen]);

//     return (
//         <div className="px-3 w-full ">
//             {/* <SliderNav
//                 flipbookRef={flipbookRef}
//                 pdfDetails={pdfDetails}
//                 viewerStates={viewerStates}
//                 screenWidth={screenWidth}
//             /> */}
//             <div className="flex items-center gap-2 pt-2 pb-2 max-xl:pt-2">
//                 <div className="hidden lg:block flex-1"></div>
//                 <Button
//                     onClick={() => { screenWidth < 768 ? flipbookRef.current.pageFlip().turnToPrevPage() : flipbookRef.current.pageFlip().flipPrev() }}
//                     disabled={viewerStates.currentPageIndex === 0}
//                     variant='secondary'
//                     size='icon'
//                     className='size-8 min-w-8'
//                 >
//                     <ChevronLeft className="size-4 min-w-4" />
//                 </Button>
//                 <Button
//                     onClick={() => { screenWidth < 768 ? flipbookRef.current.pageFlip().turnToNextPage() : flipbookRef.current.pageFlip().flipNext() }}
//                     disabled={viewerStates.currentPageIndex === pdfDetails?.totalPages - 1 || viewerStates.currentPageIndex === pdfDetails?.totalPages - 2}
//                     variant='secondary'
//                     size='icon'
//                     className='size-8 min-w-8'
//                 >
//                     <ChevronRight className="size-4 min-w-4" />
//                 </Button>
//                 <Zoom zoomScale={viewerStates.zoomScale} screenWidth={screenWidth} />
//                 {/* {!disableShare && <Share shareUrl={shareUrl} />} */}
//                 <Button
//                     onClick={fullScreen}
//                     variant='secondary'
//                     size='icon'
//                     className='size-8 min-w-8'
//                 >
//                     {screenfull.isEnabled && screenfull.isFullscreen ?
//                         <Minimize className="size-4 min-w-4" /> :
//                         <Maximize className="size-4 min-w-4" />
//                     }
//                 </Button>
//                 <div className="flex-1"></div>
//                 {pdfDetails?.totalPages > 0 && (
//                     <p className='text-sm font-medium'>
//                         {pagesInFlipView} of {pdfDetails?.totalPages}
//                     </p>
//                 )}
//             </div>
//         </div>
//     );
// };

// export default Toolbar;


import React, { useEffect, useCallback, useMemo } from 'react'
import { Button } from '../../button'
import { ChevronLeft, ChevronRight, Maximize, Minimize } from 'lucide-react'
import keyboardjs from 'keyboardjs'
import Zoom from './zoom'
import useScreenSize from '@/app-pdf/_hooks/use-screensize'

const Toolbar = ({ flipbookRef, containerRef, screenfull, pdfDetails, viewerStates, shareUrl, disableShare }) => {
  const { width: screenWidth } = useScreenSize()

  const pagesInFlipView =
    (viewerStates.currentPageIndex + 1) % 2 === 0 &&
    viewerStates.currentPageIndex + 1 !== pdfDetails.totalPages
      ? `${viewerStates.currentPageIndex + 1} - ${viewerStates.currentPageIndex + 2}`
      : viewerStates.currentPageIndex + 1

  const canFullscreen = !!screenfull && screenfull.isEnabled && typeof screenfull.toggle === 'function'

  const fullScreen = useCallback(() => {
    if (!canFullscreen) return
    const el = containerRef?.current
    if (!el) return
    screenfull.toggle(el, { navigationUI: 'hide' })
  }, [canFullscreen, screenfull, containerRef])

  // Attach error listener once (only when supported)
  useEffect(() => {
    if (!screenfull || typeof screenfull.on !== 'function' || typeof screenfull.off !== 'function') return
    const onError = (event) => {
      console.error('[screenfull] fullscreen error:', event)
    }
    screenfull.on('error', onError)
    return () => screenfull.off('error', onError)
  }, [screenfull])

  // Keyboard shortcuts
  useEffect(() => {
    const flip = flipbookRef?.current?.pageFlip?.()
    if (!flip) return

    const handleRight = () => flip.flipNext()
    const handleLeft = () => flip.flipPrev()

    keyboardjs.bind('right', null, handleRight)
    keyboardjs.bind('left', null, handleLeft)

    return () => {
      keyboardjs.unbind('right', null, handleRight)
      keyboardjs.unbind('left', null, handleLeft)
    }
  }, [flipbookRef])

  return (
    <div className="px-3 w-full">
      <div className="flex items-center gap-2 pt-2 pb-2 max-xl:pt-2">
        <div className="hidden lg:block flex-1" />

        <Button
          onClick={() => {
            const flip = flipbookRef?.current?.pageFlip?.()
            if (!flip) return
            screenWidth < 768 ? flip.turnToPrevPage() : flip.flipPrev()
          }}
          disabled={viewerStates.currentPageIndex === 0}
          variant="secondary"
          size="icon"
          className="size-8 min-w-8"
        >
          <ChevronLeft className="size-4 min-w-4" />
        </Button>

        <Button
          onClick={() => {
            const flip = flipbookRef?.current?.pageFlip?.()
            if (!flip) return
            screenWidth < 768 ? flip.turnToNextPage() : flip.flipNext()
          }}
          disabled={
            viewerStates.currentPageIndex === pdfDetails?.totalPages - 1 ||
            viewerStates.currentPageIndex === pdfDetails?.totalPages - 2
          }
          variant="secondary"
          size="icon"
          className="size-8 min-w-8"
        >
          <ChevronRight className="size-4 min-w-4" />
        </Button>

        <Zoom zoomScale={viewerStates.zoomScale} screenWidth={screenWidth} />

        {/* Fullscreen button: show only if supported */}
        {canFullscreen && (
          <Button onClick={fullScreen} variant="secondary" size="icon" className="size-8 min-w-8">
            {screenfull.isFullscreen ? <Minimize className="size-4 min-w-4" /> : <Maximize className="size-4 min-w-4" />}
          </Button>
        )}

        <div className="flex-1" />

        {pdfDetails?.totalPages > 0 && (
          <p className="text-sm font-medium">
            {pagesInFlipView} of {pdfDetails.totalPages}
          </p>
        )}
      </div>
    </div>
  )
}

export default Toolbar
