...
export default function BrochureForm(props) {
  ...
  const [debug, setDebug] = useState({
    enabled: true, // 🔧 flip to false when done
    lastTokenLen: 0,
    lastApiStatus: null,
    lastApiBody: '',
    lastError: '',
    grecaptchaPresent: false,
  })
    ...
    >
      {status.ok ? (
        ...
        >
          ...
          {debug.enabled && (
            <div className="mt-4 text-left text-xs bg-neutral-50 border border-neutral-200 rounded-xl p-3 whitespace-pre-wrap">
              <div className="font-semibold mb-1">Debug (client)</div>
              <div>siteKey present: {String(!!siteKey)}</div>
              <div>grecaptcha present: {String(debug.grecaptchaPresent)}</div>
              <div>last token length: {debug.lastTokenLen}</div>
              <div>last api status: {String(debug.lastApiStatus)}</div>
              <div>last api body: {debug.lastApiBody?.slice(0, 400) || ''}</div>
